...
<%  int id = Integer.parseInt(AccountManager.getInstance().getAccount((String)request.getSession().getAttribute("user")).getId());
	...
	Integer hotel_id = hm.addHotel(request.getParameter("name"),  Integer.parseInt(request.getParameter("stars")), request.getParameter("image"), request.getParameter("status"), request.getParameter("number"), id);
